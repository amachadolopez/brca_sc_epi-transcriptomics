...
# test <- sapply(scChIP_list, SingleCellExperiment)
```
...
scChIP_list_filtered <- sapply(scChIP_list, rm_QC)
```
...
norm_counts <- function(scChIP_df){
  mean_reads_per_cell <- mean(colSums(scChIP_df))
  scChIP_df <- sweep(scChIP_df,2, colSums(scChIP_df),"/")
  scChIP_df <- sweep(scChIP_df,2, mean_reads_per_cell,"*")
  scChIP_df <- as.data.frame(scChIP_df)
  return(scChIP_df)
}

scChIP_list_filtered2 <- sapply(scChIP_list_filtered, norm_counts)

# Mouse and human cells are normalized separately
# scChIP_list_filtered
```

# Feature annotation

```{r}
# Download marts for annotation
attributes <- c("ensembl_gene_id","start_position","end_position","hgnc_symbol","chromosome_name")
mart_hg38 <- useMart("ensembl") %>% useDataset("hsapiens_gene_ensembl", .)
mart_hg38 <- getBM(attributes=attributes, mart=mart_hg38)

mart_mm10 <- useMart("ensembl") %>% useDataset("mmusculus_gene_ensembl", .)
mart_mm10 <- getBM(attributes=attributes, mart=mart_mm10)

# Create the dataset for hg38 windows
features_hg38 <- data.frame(window=row.names(scChIP_list$`HBCx-95_scChIP_H3K27me3_hg38`)) %>% 
  mutate(chromosome=str_remove((str_split(.$window, "_", simplify = T)[,1]),"chr"),
         start=(str_split(.$window, "_", simplify = T)[,2]),
         end=(str_split(.$window, "_", simplify = T)[,3]))


# Create the dataset for mm10 windows
features_mm10 <- data.frame(window=row.names(scChIP_list$`HBCx-95_scChIP_H3K27me3_mm10`)) %>% 
  mutate(chromosome=str_remove((str_split(.$window, "_", simplify = T)[,1]),"chr"),
         start=(str_split(.$window, "_", simplify = T)[,2]),
         end=(str_split(.$window, "_", simplify = T)[,3]))
